...
First, we can observe that there is 3 countries that look alike, where their democracy indexes did not change a lot over the years and stayed high: France, United states and Israel. 
Second, the 2 other countries, Cameroon and Morocco, looked alike from the year 2006 to 2014, but from the year 2015 they changed in the opposite way. Morocco began te be a little bo democratic an Cameroon a little less.
...
# Renaming columns of the tables
...
We can see the first 5 countries in this new table with all the necessaries information added, like the rate of incarnation or the number of population for every countries
...
On a second hand, the second plot that analyse the connection between the democracy index and the incarnation rate doesn't show a real connection. The regression line is almost horizontal, the points are scattered and not really explaning the regression line.

6.a. 
```{r, cache=TRUE} 
colnames(gdp_table)[colnames(gdp_table) == "CIA.8..9..10."] <- "CIA.est"
colnames(gdp_table)[colnames(gdp_table) == "CIA.8..9..10..1"] <- "CIA.year"

x <- na.omit(as.numeric(gdp_table$CIA.est))
x_ecdf <- ecdf(x)
plot(x_ecdf, xlab ="x", ylab="ECDF", main= "Empirical CDF", col.main="blue", pch = 16)
```

On this plot we can see the cdf of gdp per capita of a randomly selected group of a 1,000 countries out of all the counties data.
the X axis on the graph represents the GDP per capita, and the Y axis represents the Empirical cdf. 
We can also see where x is located on that scale, where x is also a randomly selected country.
...
gdp_pers<-joined_table%>%select(Country,CIA.est,Population)
gdp_pers<-na.omit(gdp_pers)

#Transforming the vector gdp_area$Landin.km2..mi2. to numeric and then rounding the values to the nearest integer
gdp_pers$Population<-round(gdp_pers$Population)

#Step 2 --> Calculating for every country the probability of selecting a person from that country. This will then be the fourth column in the data-base `gdp_area`. 

gdp_pers<-gdp_pers%>%mutate(Weights=gdp_pers$Population/sum(gdp_pers$Population))


#Step 3 --> Calculating the weighted empirical CDF function with the function `ewcdf()`. This function simply calculates the ecdf of the vector `gdp_area$CIA.8..9..10.` but weights every value with the vector `gdp_area$Weights`.

ecdf_Y<-ewcdf(gdp_pers$CIA.est,weights=gdp_pers$Weights)

plot(ecdf_Y,xlab="GDP (PPP) per capita in Int$",ylab="EWCDF",main="EW-CDF of GDP per capita of a randomly selected person",sub="(Weighted by Population of a person's country)" ,col.main="blue",col.sub="blue",pch=16,verticals=TRUE,do.points=FALSE)
```

The sampled gdp person variable ( AKA Y) represents the Gdp of a randomly uniformly selected person out of all the countries.
...
# Remove numbers in parentheses from LandArea column
joined_table$LandArea <- gsub("\\s*\\(.*\\)", "", joined_table$LandArea)

# Convert LandArea column to numeric format
joined_table$LandArea <- as.numeric(joined_table$LandArea)

#Step 1 --> Creating a new data-frame `gdp_area` with three columns: one with the name of a country, one with the GDP per capita of that country and one with the land-area of that country. Then removing all NA values. Also transforming the vector gdp_area$Landin.km2..mi2. to numeric and rounding the values to the neareast integer.

gdp_area<-joined_table%>%select(Country,CIA.est,LandArea)
gdp_area<-na.omit(gdp_area)
...
gdp_area<-gdp_area%>%mutate(Weight=gdp_area$LandArea/sum(gdp_area$LandArea))


#Step 3 --> Calculating the weighted empirical CDF function with the function `ewcdf()`. This function simply calculates the ecdf of the vector `gdp_area$CIA.8..9..10.` but weights every value with the vector `gdp_area$Weights`.

ecdf_Z<-ewcdf(gdp_area$CIA.est,weights=gdp_area$Weight)

plot(ecdf_Z,xlab="GDP (PPP) per capita in Int$",ylab="EWCDF",main="EW-CDF of GDP per capita of a randomly selected person",sub="(Weighted by land-area of a person's country)" ,col.main="blue",col.sub="blue",pch=16,verticals=TRUE,do.points=FALSE)

# Compare median, 25th percentile, and 75th percentile of X, Y, and Z
median_X <- median(x)
median_Y <- quantile(gdp_pers$CIA.est, probs = 0.5, weights = gdp_pers$Weights)
median_Z <- quantile(gdp_area$CIA.est, probs = 0.5, weights = gdp_area$Weight)

percentile_25_X <- quantile(x, 0.25)
percentile_25_Y <- quantile(gdp_pers$CIA.est, probs = 0.25, weights = gdp_pers$Weights)
percentile_25_Z <- quantile(gdp_area$CIA.est, probs = 0.25, weights = gdp_area$Weight)

percentile_75_X <- quantile(x, 0.75)
percentile_75_Y <- quantile(gdp_pers$CIA.est, probs = 0.75, weights = gdp_pers$Weights)
percentile_75_Z <- quantile(gdp_area$CIA.est, probs = 0.75, weights = gdp_area$Weight)
...
We calculated the average democracy score between 2006 and 2022 for each country.
then we created a scale representing the scores- from the lowest average (1.06) and colored it close to white, the higest (9.83) as red.
the countries received colors based on the scale colors (white to red).
By reviewing the results we can say that countries on the West side of the map are the most democratic.
Countries in the South-EAST edge and Europe  are very democratic as well.
On the other hand, Africa, the middle east and the East-South countries have low democratic scores (with exceptions).
...
In the first data table we have presented the top five.

In the heatmap of the five democracy elements correlations we can see that the 2 pairs -  Electoral process and pluralism (EEP) and Civil liberties (CL), Functioning of government(FG) and Civil liberties,have the highest  correlations (above 0.8).
We can say that the following pairs have highest correlations : (EPP ,FG),(EPP ,Political Prticipation(PP)), (PP, CL).
The lowest pairs are: (EPP,Political culture) and (Political Participation,Political culture).

Generally speaking, if the color is darker than it has a stronger correlati
