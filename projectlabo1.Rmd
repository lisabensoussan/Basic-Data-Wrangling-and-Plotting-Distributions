...
reg_mod_cia <- lm(`CIA.est` ~ X2022, data = joined_table)
summary(reg_mod_cia)

ggplot(joined_table, aes(x = X2022, y = `CIA.est`)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Democracy Index 2022", y = "GDP (PPP) per capita") +
  ggtitle("Scatter Plot of GDP vs. Democracy Index")


colnames(joined_table)[colnames(joined_table) == "Rate.per.100.000..3."] <- "Rate.100000"
reg_mod_inca <- lm(`Rate.100000` ~ X2022, data = joined_table)
summary(reg_mod_inca)

ggplot(joined_table, aes(x = X2022, y = `Rate.100000`)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Democracy Index 2022", y = "Rate incarceration (per 100 000") +
  ggtitle("Scatter Plot Rate incarceration vs. Democracy Index")
```
