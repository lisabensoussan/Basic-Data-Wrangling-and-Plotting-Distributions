...
# Filter out non-finite values in GDP per capita and population CDF
filtered_data <- na.omit(joined_table[, c("CIA.est", "Population")])

# Compute the cumulative sum of the population size
population_cdf <- cumsum(filtered_data$Population) / sum(filtered_data$Population)

# Compute the empirical CDF using the filtered GDP per capita values
ecdf_person <- ecdf(filtered_data$CIA.est)

# Plot the empirical CDF
ggplot(data.frame(GDP = filtered_data$CIA.est, PopulationCDF = population_cdf), aes(x = GDP, y = PopulationCDF)) +
  stat_ecdf() +
...
# Filter out non-finite values in Land Area and GDP per capita
filtered_dat <- na.omit(joined_table[, c("LandArea", "CIA.est")])

# Compute the cumulative sum of land area
land_area_cdf <- cumsum(filtered_dat$Land.Area) / sum(filtered_dat$Land.Area)

# Compute the empirical CDF for Z using land area
ecdf_location <- ecdf(filtered_dat$CIA.est)

# Generate a sequence of values for plotting the ECDF
x <- seq(0, 200000, length.out = 1000)

# Create a data frame for plotting
df <- data.frame(LandArea = x, CDF = ecdf_location(x))

# Plot the empirical CDF for Z
library(ggplot2)
ggplot(df, aes(x = LandArea, y = CDF)) +
  geom_step() +
  labs(x = "Land Area (km^2)", y = "Empirical CDF", title = "Empirical CDF of GDP per capita based on Land Area")

# Compare median, 25th percentile, and 75th percentile of X, Y, and Z



# Calculate the median of x_ecdf if it is a numeric vector
median_X <- median(x)
median_Y <- median(filtered_dat$CIA.est)
median_Z <- median(filtered_dat$CIA.est/filtered_dat$Land.Area)

percentile_25_X <- quantile(x, 0.25, na.rm = TRUE)
percentile_25_Y <- quantile(filtered_dat$CIA.est, 0.25)
percentile_25_Z <- quantile(filtered_dat$CIA.est / filtered_dat$Land.Area, 0.25)

percentile_75_X <- quantile(x, 0.75, na.rm = TRUE)
percentile_75_Y <- quantile(filtered_dat$CIA.est, 0.75)
percentile_75_Z <- quantile(filtered_dat$CIA.est / filtered_dat$Land.Area, 0.75)
